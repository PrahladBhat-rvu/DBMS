...
select department_id, count(*) as num_emp from departments group by department_id;

-- Paste the output of above code and comment your output
/*
	department_id	num_emp
	10	1
	20	1
	30	1
	40	1
...
-- Write the SQL Statement for the above Question
select job_id, avg(salary) as averagesalary from employees group by job_id;
...
-- Write the SQL Statement for the above Question
select d.department_name, e.department_id, sum(salary) as 'tot_sal' from employees e, departments d where  e.department_id =  d.department_id group by department_id having tot_sal > 50000;

-- Paste the output of above code and comment your output
/*
	department_name	department_id	tot_sal
	Executive	90	58000.00
	Finance	100	51600.00
	Shipping	50	156400.00
	Sales	80	304500.00
*/
...
-- Write the SQL Statement for the above Question
select department_id, max(salary) as max_salary, min(salary) as min_salary, avg(salary) as avg_salary from employees group by department_id having max(salary) >= 10000;
...
-- Write the SQL Statement for the above Question
select extract(year from hire_date) AS hire_year, count(*) employee_count from employees group by extract(year from hire_date) order by hire_year;
...
-- Write the SQL Statement for the above Question
select e.job_id, j.job_title, avg(e.salary) as average_salary from employees e join jobs j on e.job_id = j.job_id group by e.job_id, j.job_title having avg(e.salary) > 8000;
...
-- Write the SQL Statement for the above Question
SELECT department_id, avg(commission_pct) as avg_commission_pct from employees group by department_id having avg(commission_pct) is not null;
...
-- Write the SQL Statement for the above Question
select manager_id, count(*) as employee_count from employees group by manager_id order by employee_count desc;
...
-- Write the SQL Statement for the above Question
select department_id, job_id, sum(salary) as total_salary from employees group by department_id, job_id order by department_id asc, total_salary desc;
